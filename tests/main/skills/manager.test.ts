...
/**
 * SkillManager 行为契约 (基于计划二 §八):
 *
 * scanAll:
 * - 扫描 skills 目录下所有子目录中的 SKILL.md
 * - 从 SKILL.md 解析 name (# heading) 和 description (heading 后首行非空文本)
 * - 目录不存在时自动创建，返回空数组
 * - readdir 返回字符串数组（目录名），代码无 isDirectory 过滤
 *
 * buildCatalog:
 * - 将 SkillMeta[] 格式化为可注入 system prompt 的字符串
 * - 空数组返回空字符串
 * - 包含每个 skill 的 id, name, description
 */
interface MockFs {
...
describe("SkillManager", () => {
  ...
  async function getManager() {
    vi.resetModules();
    const mod = await import("../../../src/main/skills/manager");
